...
OUTPUT_DIR = './output/pose/example'

# for validation set use these paths:
...
def main():
    ...
    # !!!! ce zelis izlocit vmesno pozo spremeni tu spodaj pose_out3_x v pose_out1_x -> this works!!!
    pose_out3 = tf.reduce_mean(tf.stack([tf.image.resize_nearest_neighbor(pose_out1_100, tf.shape(image_batch_origin)[1:3,]),
                                         tf.image.resize_nearest_neighbor(pose_out2_125, tf.shape(image_batch_origin)[1:3,]),
